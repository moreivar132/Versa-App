exports.up = async function (knex) {
  ...
  `);
    ...
    // =========================================================================
    // TABLAS RBAC ADICIONALES (requeridas por migraciones posteriores)
    // =========================================================================
    await knex.schema.createTable('usuariorol', (table) => {
        table.increments('id').primary();
        table.integer('id_usuario').notNullable();
    ...
    await knex.schema.createTable('rolpermiso', (table) => {
        table.increments('id').primary();
        table.integer('id_rol').notNullable();
        table.integer('id_permiso').notNullable();
        table.timestamp('created_at', { useTz: false }).defaultTo(knex.fn.now());
    });

    // =========================================================================
    // TABLA ESTADOORDEN (requerida por migraciones de orden_trabajo)
    // =========================================================================
    await knex.schema.createTable('estadoorden', (table) => {
        table.increments('id').primary();
    ...
    await knex.schema.alterTable('usuariorol', (table) => {
        table.foreign('id_usuario').references('id').inTable('usuario');
        table.foreign('id_rol').references('id').inTable('rol');
        table.foreign('id_tenant').references('id').inTable('tenant');
    });

    await knex.schema.alterTable('rolpermiso', (table) => {
        table.foreign('id_rol').references('id').inTable('rol');
        table.foreign('id_permiso').references('id').inTable('permiso');
    });
