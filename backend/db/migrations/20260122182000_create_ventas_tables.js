/**
 * Migration: create_ventas_tables
 * Source: backend/archive/legacy-migrations/create_ventas_tables.sql
 * Module: Manager
 * Risk Level: Medio
 * 
 * Creates sales module tables:
 * - venta: main sales table
 * - ventalinea: sale line items
 * - ventapago: sale payments
 */

exports.up = async function (knex) {
    console.log('[Migration] Creating ventas tables...');
    ...
    await knex.raw(`
        ...
        COMMENT ON TABLE venta IS 'Ventas directas de productos';
        COMMENT ON TABLE ventalinea IS 'Líneas de detalle de ventas';
        COMMENT ON TABLE ventapago IS 'Pagos asociados a una venta';
    `);

    console.log('[Migration] ✅ Ventas tables created');
};

exports.down = async function (knex) {
    console.log('[Migration] 🗑️ Dropping ventas tables...');

    await knex.raw(`
        DROP TABLE IF EXISTS ventapago CASCADE;
        DROP TABLE IF EXISTS ventalinea CASCADE;
        DROP TABLE IF EXISTS venta CASCADE;
    `);

    console.log('[Migration] ✅ Ventas tables dropped');
};
