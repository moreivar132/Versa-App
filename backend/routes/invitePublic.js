...
router.post('/accept', async (req, res) => {
    ...
    try {
        ...
        if (invite.used_at) {
            await client.query('ROLLBACK');
            return res.status(400).json({ ok: false, error: 'Esta invitación ya fue utilizada' });
        }

        if (new Date(invite.expires_at) < new Date()) {
            await client.query('ROLLBACK');
            return res.status(400).json({ ok: false, error: 'Esta invitación ha expirado' });
        }

        // Determine the email to use
        let userEmail;
        if (invite.email_allowed) {
            // Email is locked to invite
        ...
        console.log(`[InvitePublic] User created via invite: ${userEmail} (tenant: ${invite.tenant_id}, role: ${invite.role})`);
        ...
        res.status(500).json({ ok: false, error: 'Error al crear cuenta' });
    } finally {
        client.release();
    }
});
