...
router.get('/', verifyJWT, async (req, res) => {
    try {
        const db = getTenantDb(req.user); // Use tenant DB from user context
...
router.get('/:id/movimientos', verifyJWT, async (req, res) => {
    try {
        ...
        const mecanicoCheck = await db.query(
        ...
        const totalesResult = await db.query(totalesSql, totalesParams);
...
router.post('/:id/movimientos', verifyJWT, async (req, res) => {
    ...
    try {
        ...
        await db.tx(async (client) => {
            ...
            if (mecanicoCheck.rows.length === 0) {
                throw { status: 404, message: 'Mecánico no encontrado' };
            }
            ...
            if (!tiposValidos.includes(tipo)) {
                throw { status: 400, message: 'Tipo de movimiento inválido' };
            }

            if (!monto || parseFloat(monto) <= 0) {
                throw { status: 400, message: 'Monto debe ser mayor a 0' };
            }

            if (!id_medio_pago) {
                throw { status: 400, message: 'Debe especificar el método de pago' };
            }
            ...
            if (medioPagoCheck.rows.length === 0) {
                throw { status: 400, message: 'Método de pago no válido' };
            }
...
router.get('/:id/resumen', verifyJWT, async (req, res) => {
    try {
        ...
        const ordenesResult = await db.query(ordenesQuery, ordenesParams);
        ...
        const movResult = await db.query(movQuery, movParams);
