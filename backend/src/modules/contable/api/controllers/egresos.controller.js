    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            await req.db.txWithRLS(async (tx) => {
                ...
                const intakeResult = await tx.query(`
                ...
                if (ocrError) {
                    ...
                    await tx.query('SAVEPOINT create_borrador');
                    try {
                        ...
                        if (!contactoId && provNombre) {
                            const newContact = await tx.query(`
                        ...
                        // Create factura
                        const facturaRes = await tx.query(`
                        ...
                        await tx.query('RELEASE SAVEPOINT create_borrador');
                    } catch (borradorError) {
                        await tx.query('ROLLBACK TO SAVEPOINT create_borrador');
                        console.error(`[Egresos] ⚠️ Error creating borrador (recovered):`, borradorError.message);
                        gastoId = null;
                    }
                }

                // Response is sent AFTER transaction commits (automatically by txWithRLS on success)
                res.status(201).json({
                    ok: true,
                    intake_id: intakeId,
                    status: dbStatus,
...
async function getIntake(req, res) {
    try {
        ...
        const result = await req.db.query(`SELECT * FROM accounting_intake WHERE id = $1 AND id_tenant = $2`, [intakeId, tenantId]);
        ...
        if (intake.status !== 'processing') {
            const draft = await req.db.query(
    ...
    try {
        ...
        await req.db.txWithRLS(async (tx) => {
            ...
            if (!contactoId && proveedor_nombre) {
                if (proveedor_nif) {
                    const provCheck = await tx.query(
                        'SELECT id FROM contabilidad_contacto WHERE id_tenant=$1 AND nif_cif=$2 LIMIT 1',
            ...
            if (finalId) {
                ...
                if (dupCheck.rows.length > 0) {
                    const err = new Error(`Ya existe la factura ${numero_factura} para este proveedor.`);
...
async function listGastos(req, res) {
    try {
        ...
        const countRes = await req.db.query(countQuery, countParams);
